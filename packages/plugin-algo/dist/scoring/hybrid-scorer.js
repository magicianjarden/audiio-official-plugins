...
exports.HybridScorer = void 0;
const ml_sdk_1 = require("@audiio/ml-sdk");
class HybridScorer {
    constructor(endpoints, neuralScorer, settings) {
     ...
     */
    async score(track, features, context) {
        ...
        const finalScore = (0, ml_sdk_1.calculateWeightedScore)(components, weights);
        // Generate explanation
        const explanation = (0, ml_sdk_1.generateExplanation)(components);
    ...
    async scoreBatch(tracks, context) {
        ...
        // Score all tracks
        const scorePromises = tracks.map((track, i) => this.score(track, features[i], context));
        ...
        for (const [key, value] of Object.entries(cached.components)) {
            if (value !== undefined) {
                details.push({
                    ...
                    reason: this.getComponentReason(key, value),
                });
            }
        }
     ...
     * Calculate all score components
     */
    async calculateComponents(track, features, context) {
        const components = {};
        // === Base Preference ===
        ...
        if (features.audio && context.currentTrack) {
            ...
            if (currentFeatures) {
                components.audioMatch = (0, ml_sdk_1.calculateAudioMatchScore)(features.audio, currentFeatures) * 100;
            }
        }
        // === Mood Match ===
        ...
        if (features.audio?.key && context.currentTrack) {
            ...
            if (currentFeatures?.key && features.audio.mode && currentFeatures.mode) {
                components.harmonicFlow = (0, ml_sdk_1.getHarmonicCompatibility)(features.audio.key, features.audio.mode, currentFeatures.key, currentFeatures.mode) * 100;
            }
        }
        ...
        if (this.settings.enableTemporalMatching !== false) {
            ...
            const timeOfDayMode = this.settings.timeOfDayMode || 'auto';
            if (timeOfDayMode === 'off') {
                ...
                const energyHistory = this.temporalPatterns?.energyByHour ?? ml_sdk_1.DEFAULT_ENERGY_CURVE;
                const enhancedResult = (0, ml_sdk_1.calculateEnhancedTemporalFit)(context.hourOfDay, energy, valence, energyHistory);
        ...
        if (this.settings.enableSessionFlow !== false && context.sessionTracks.length > 0) {
            ...
            components.sessionFlow = (0, ml_sdk_1.calculateSessionFlowScore)(trackEnergy, sessionEnergies) * 100;
        }
        // === Activity Match ===
        ...
        const explorationLevel = this.settings.explorationLevel || 'balanced';
        const epsilon = explorationLevel === 'high' ? 0.25 : explorationLevel === 'low' ? 0.05 : 0.15;
        components.explorationBonus = (0, ml_sdk_1.calculateExplorationBonus)(isNewArtist, isNewGenre, 0, // TODO: Get actual play counts
        0, epsilon) * 100;
        // === Serendipity ===
        ...
        components.serendipityScore = (0, ml_sdk_1.calculateSerendipityScore)(track, topGenres, topArtists, isNewArtist) * 100;
        // === Diversity ===
        components.diversityScore = (0, ml_sdk_1.calculateDiversityScore)(track, context.sessionArtists, context.sessionGenres) * 100;
        // === Penalties ===
        const lastPlayed = await this.endpoints.user.getLastPlayed(track.id);
        components.recentPlayPenalty = (0, ml_sdk_1.calculateRecentPlayPenalty)(lastPlayed);
        // Dislike penalty
        ...
        const artistDisliked = dislikedTracks.some(d => d.artistId === track.artistId && d.reason === 'dont_like_artist');
     ...
     */
    getWeights() {
        const mlWeight = this.neuralScorer.getMLWeight() * (this.settings.mlWeight || 0.5);
     ...
     */
    calculateMoodMatch(emotion, targetMood) {
     ...
     */
    calculateActivityMatch(audio, activity) {
    ...
    async ensurePreferences() {
        if (Date.now() < this.preferencesExpiry)
            return;
