...
export class YouTubeVideosProvider extends BaseArtistEnrichmentProvider {
  ...
  get manifest(): AddonManifest {
    ...
    version: '1.0.0',
    description: 'Music videos from YouTube',
    author: 'Audiio',
      ...
      dataAccess: [
        {
          ...
          userFriendlyDesc: 'Artist and album names used to find videos',
          technicalDesc: 'Search queries for video discovery (artist + "official music video")'
        },
        {
          category: 'audio-content',
          usage: ['service-functionality'],
          required: true,
          userFriendlyLabel: 'Video Streaming',
          userFriendlyDesc: 'Streams video content from YouTube',
          technicalDesc: 'Video stream URLs fetched via youtubei.js library'
        }
      ],
      ...
      networkAccess: [
        {
          ...
          purpose: 'Search and stream music videos',
          dataTypes: ['library-data', 'audio-content']
        },
        {
          host: 'i.ytimg.com',
          purpose: 'Fetch video thumbnails',
          dataTypes: ['library-data']
        }
      ],

      localStorageUsed: true,
      localStorageDesc: 'Caches video search results for 30 minutes',
  ...
  async getVideoStream(videoId: string, preferredQuality = '720p'): Promise<VideoStreamInfo | null> {
    ...
    try {
      ...
      // Type for format (same as YTMusic)
      type Format = {
        url?: string;
        mime_type?: string;
        itag?: number;
      ...
      try {
        ...
        if (musicInfo) {
          ...
          if (streamingData?.adaptive_formats) {
            ...
            const videoFormats = (streamingData.adaptive_formats as Format[])
              .filter(f => f.mime_type?.includes('video') && f.url && !f.signatureCipher)
              .sort((a, b) => (b.height || 0) - (a.height || 0));

            const audioFormats = (streamingData.adaptive_formats as Format[])
      ...
      try {
        ...
        if (streamingData) {
          console.log('[YouTube Videos] Streaming data:',
            'formats:', streamingData.formats?.length || 0,
            'adaptive:', streamingData.adaptive_formats?.length || 0
          );
          ...
          if (streamingData.adaptive_formats) {
            ...
            for (const format of streamingData.adaptive_formats) {
              const f = format as unknown as Format;
              if (f.url) withUrl++;
              else if (f.signatureCipher) withCipher++;
              else noUrl++;
            }
            ...
            console.log('[YouTube Videos] Available video qualities:',
              availableVideos.map(v => v.quality_label || `${v.height}p`).join(', ')
            );
          ...
          if (streamingData.formats) {
            ...
            const combinedFormats = (streamingData.formats as unknown as Format[])
