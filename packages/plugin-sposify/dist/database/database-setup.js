 ...
 */
function checkDatabaseExists(userDataPath) {
    ...
    try {
        ...
        const result = db.prepare("SELECT value FROM metadata WHERE key = 'version'").get();
        db.close();
        return {
            exists: true,
            path: dbPath,
 ...
 */
async function downloadDatabase(destPath, onProgress) {
 ...
 */
function downloadFile(url, destPath, compressed, onProgress) {
 ...
 */
async function decompressFile(sourcePath, destPath, onProgress) {
    const sourceStats = fs.statSync(sourcePath);
    let processedBytes = 0;
    const source = (0, fs_1.createReadStream)(sourcePath);
    const gunzip = zlib.createGunzip();
    const dest = (0, fs_1.createWriteStream)(destPath);
    ...
    await (0, promises_1.pipeline)(source, gunzip, dest);
}
/**
 * Build sample database for offline/testing
 */
async function buildSampleDatabase(destPath, onProgress) {
    onProgress?.({
        phase: 'building',
        progress: 0,
    ...
    `);
        db.transaction(() => {
            for (let i = 0; i < sampleData.tracks.length; i++) {
                ...
                insertTrack.run(t.id, t.title, t.artist, t.artistId, t.album, t.duration, t.isrc, t.popularity, t.explicit ? 1 : 0, JSON.stringify(t.genres));
                insertFeatures.run(t.id, t.tempo, t.key, t.mode, t.danceability, t.energy, t.loudness, t.speechiness, t.acousticness, t.instrumentalness, t.liveness, t.valence);
                if (i % 1000 === 0) {
                    onProgress?.({
                        phase: 'building',
