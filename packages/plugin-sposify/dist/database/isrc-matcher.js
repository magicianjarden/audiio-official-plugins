...
exports.IsrcMatcher = void 0;
exports.getIsrcMatcher = getIsrcMatcher;
...
class IsrcMatcher {
    ...
    matchSingleTrack(track) {
        ...
        for (const match of exactMatches) {
            if (this.normalizeString(match.title) === this.normalizeString(track.title) &&
        ...
        for (const match of exactMatches) {
            const titleSimilarity = this.calculateSimilarity(this.normalizeString(track.title), this.normalizeString(match.title));
            const artistSimilarity = this.calculateSimilarity(this.normalizeString(track.artist), this.normalizeString(match.artist_name));
    ...
    fuzzyMatch(track) {
        ...
        if (searchResults.length === 0)
            return null;
        // Build search index
        const fuse = new fuse_js_1.default(searchResults, {
            keys: [
     ...
     */
    createMatch(localId, dbTrack, confidence, matchedBy) {
        return {
            localId,
    ...
    setFuzzyMatchingEnabled(enabled) {
        this.enableFuzzyMatching = enabled;
    }
    setDurationTolerance(ms) {
        this.durationToleranceMs = ms;
    }
}
exports.IsrcMatcher = IsrcMatcher;
// Singleton
let instance = null;
function getIsrcMatcher() {
    if (!instance) {
        instance = new IsrcMatcher();
    }
    return instance;
}
//# sourceMappingURL=isrc-matcher.js.map